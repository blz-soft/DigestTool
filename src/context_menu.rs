...
fn set_key_to_directory_background(post_path: &str) -> Result< (), std::io::Error> {
    ...
    .args(&[r"New-Item", &format!("{}{}",pre_path, post_path), r"-Force"])
    .output()?;
    if !result_set_key.status.success() {
        return Err(std::io::Error::new(std::io::ErrorKind::Other, "レジストリの編集ができませんでした"));
    };
    Ok(())
}
...
fn set_property_to_directory_background(post_path: &str, name: &str, value: &str) -> Result< (), std::io::Error> {
    let pre_path = r"HKCU:SOFTWARE\Classes\Directory\Background\shell\";
    let result_set_key = std::process::Command::new("powershell.exe")
    .args(&[r"New-ItemProperty", r"-LiteralPath", &format!("{}{}",pre_path, post_path), r"-Name", name, r"-Value", value])
    .output()?;
    if !result_set_key.status.success() {
        return Err(std::io::Error::new(std::io::ErrorKind::Other, "レジストリの編集ができませんでした"));
    }
...
/// コンテクストメニューのアイテムを追加する
/// ファイルの右クリックメニューを追加する
fn set_property_to_faile_menu(post_path: &str, name: &str, value: &str) -> Result< (), std::io::Error> {
    let pre_path = r"HKCU:SOFTWARE\Classes\*\shell\";
    let result_set_key = std::process::Command::new("powershell.exe")
    .args(&[r"New-ItemProperty", r"-LiteralPath", &format!("{}{}",pre_path, post_path), r"-Name", name, r"-Value", value])
    .output()?;
...
fn remove_key_from_dircetory_background(post_path: &str) -> Result< (), std::io::Error> {
    ...
    .args(&[r"Remove-Item",r"-LiteralPath" , &format!("{}{}", pre_path, post_path), r"-Recurse"])
    .output()?;
